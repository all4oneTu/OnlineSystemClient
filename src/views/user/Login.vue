...
<template>
  <div class="main">
    ...
    >
      ...
      >
        <a-tab-pane key="tab1" tab="Đăng nhập hệ thống">
          <a-form-item>
            <a-input
              size="large"
              type="text"
              ...
              v-decorator="[
                'username',
                {rules: [{ required: true, message: 'Tài khoản/Email/Số điện thoại' }, { validator: handleUsernameOrEmail }], validateTrigger: 'change'}
              ]"
            >
          ...
          <a-form-item>
            <a-input
              ...
              v-decorator="[
                'password',
                {rules: [{ required: true, message: 'Mật khẩu' }], validateTrigger: 'blur'}
              ]"
            >
      ...
      <a-form-item>
        <a-checkbox v-decorator="['rememberMe']">Tự động đăng nhập</a-checkbox>
        <router-link class="register" :to="{ name: 'register' }" style="float: right;">Đăng kí tài khoản</router-link>
      </a-form-item>
  ...
  methods: {
    ...
    loginSuccess (res) {
      ...
      setTimeout(() => {
        this.$notification.success({
          message: 'Xin chào',
          description: `${timeFix()}，chào mừng quay trở lại`
        })
      }, 1000)
    },
    requestFailed (err) {
      this.$notification['error']({
        message: 'Lỗi !!!',
        description: ((err.response || {}).data || {}).message || 'Tên người dùng hoặc mật khẩu sai',
        duration: 4
      })
    }
  }
